...
//   META_PIXEL_ID          ex.: 4177249519256900 (público)
//   META_CAPI_TOKEN        (SECRETO) token da Conversions API
//   META_TEST_EVENT_CODE   (opcional) ex.: TEST5322 — só para testes no Gerenciador
//
...
async function handleTrack(request, env) {
  ...
  // Sem token, não envia (mantém a UX; o pixel do navegador continua funcionando).
  if (!pixelId || !token) return json({ ok: true, configured: false });
  ...
  // Sempre 2xx para o kit (ele só checa r.ok); detalhe do erro vai no corpo.
  if (resp.ok) return json({ ok: true, configured: true });
  const err = await resp.json().catch(() => ({}));
  return json({ ok: true, configured: true, meta_error: err.error ? err.error.message : "capi_error" });
}
